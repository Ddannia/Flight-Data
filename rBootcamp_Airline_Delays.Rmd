...
Timeliness is critical for Jim, as missing meetings could have significant consequences. Therefore, our goal is to identify airlines with fewer delays. By conducting this analysis, we aim to help the company in reducing flight delays during Jim's business trips, thereby minimizing disruptions and save time and money. 
...
## 2. Data Preparation
We obtained two data sets airports.csv and flights.csv from Kaggle.com. The flights.csv file contains information about flights in 2015, including the airline, fligth date, origin and destination airports, and departure and arrival times, distance, the binary variable whether a flight was cancelled and other variables, that are not relevant to the analysis and thus, are removed from the data set. The data spans the year 2015 and include 5.8 Mio US domestic flight entries. There in total 31 variables. 
The airports.csv file complements the first file with gps coordinates of the airports and contains 322 entries with 7 variables. [^1]

[^1]: https://www.kaggle.com/datasets/usdot/flight-delays

```{r getData, echo = TRUE, include= TRUE, cache = TRUE}
...
The data set remains with ten variables:
...
``` {r checkEmptyFields, echo = TRUE, include = FALSE}
...
```
The result is `r sum(is.na (flights.cleaned))`: There is no missing value in the data set "flights.cleaned".

In the next step, we investigate the unique departure airports of all flights. By looking at the tail of the data set we find out that there are numbers indicated as airport names. 
...
```
In fact, for all flights in October, the airport names are given as digits.   
```{r OctoberFlights, echo=TRUE, include=TRUE}
...
After cleaning, we obtain a data set with `r ncol(flights.filtered)` columns and `r nrow(flights.filtered)` rows.
...
In the following, we would like to filter the airlines that operate these routes and save them in the "result.list".
...
```{r RouteCheck, echo=TRUE, include=TRUE}
...
```
The above results show that the airlines AS (Alaska) and B6 (JetBlue) have all the routes in common. We therefore remove all entries that do not correspond to the desired routes and the desired airlines names, and save the data as "flights.subset".
```{r filterAirlines, echo=TRUE, include=FALSE}
# Set the conditions regarding routes
conditions <- 
  (flights.filtered$ORIGIN_AIRPORT == "ANC" & flights.filtered$DESTINATION_AIRPORT == "PDX") |
  (flights.filtered$ORIGIN_AIRPORT == "PDX" & flights.filtered$DESTINATION_AIRPORT == "ANC") |
  (flights.filtered$ORIGIN_AIRPORT == "PDX" & flights.filtered$DESTINATION_AIRPORT == "BOS") |
...
```{r combineDataSets, echo = TRUE, include= TRUE}
...
In this sub chapter, we would like to have a look at the delay distribution of the two airlines both in 2015 and in each month of 2015. The data set contains both the DEPARTURE_DELAY and the ARRIVAL_DELAY. 
For the delay examination, we will use the variable "ARRIVAL_DELAY" instead of the "DEPARTURE_DELAY", because the first one represents the actual delay, that a flight has. 
...
Note: As the data for flights in October are removed due to erroneous data, the month October is not displayed in the plot.
...
When we look at the ridgeline plots comparing the flight delays of Alaska and JetBlue airlines, we don't see a clear difference. However, there's a subtle distinction: Alaska's delay distribution appears to have a slightly smaller center compared to JetBlue's. This suggests that JetBlue experience larger delays than Alaska on average. Also, Alaska's delay distribution seems to be closer to a normal distribution, while JetBlue's is skewed to the right. This implies that JetBlue has greater positive (arriving later than scheduled) than negative delays (arriving earlier than scheduled).

#### 3.1.3 Summary Statistics
To further investigate the distributions, we examine specific statistical values for the delays of both airlines. Specifically, we will look at the summary statistics, the standard deviation and the SEM (standard error of the mean).
...
**Standard Deviation**
```{r StandardDeviation, echo = TRUE, include= TRUE}
sd(flights.subset.AS$ARRIVAL_DELAY)
sd(flights.subset.B6$ARRIVAL_DELAY)
```

**SEM**
```{r SEM, echo = TRUE, include= TRUE}
...
The summary statistics show that Alaska Airlines (as Alaska in the following for simplicity) operated 3066  flights in 2015 within the US. JetBlue Airlines (as JetBlue in the following) operated 2612 flights in the same year regarding the desired routes.
...
The histogram shows that Alaska Airlines had a higher number of on-time flights in 2015 compared to B6, and fewer flights in the categories of small and large delays, despite having a greater total number of flights in that year (3066 vs. 2612). This suggests that Alaska Airlines, again, is more reliable in terms of flight punctuality. 

In Chapter four, we will analyze whether the delay difference between the two airlines has statistical significance.


### 3.3 Airline and flight distance 
Another interesting comparison to look at is, whether longer flights have more delays. Therefore we now compare the density of the distance of the two airlines. 

``` {r DensityPlot, include = TRUE, warning = FALSE}
...
The density plot shows that Alaska operates about the same amount of short flights (distance around 1550 km) as long flights (distance around 2500 km).In comparison, JetBlue operates more long distance flights.
```{r DelayByDistancePlot, echo=TRUE, include = TRUE, warning = FALSE}
ggplot(data = flights.with.coordinates, aes(x = DISTANCE, y = ARRIVAL_DELAY, color = AIRLINE)) +
  stat_summary(fun = mean, geom = "point", size = 3) +
  theme_classic() +
  scale_x_continuous(breaks = seq(0, 3000, by = 100)) +
  labs(title = "Mean Arrival Delay by Distance and Airline",
       x = "Distance (in miles)",
       y = "Mean Arrival Delay",
       color = "Airline")
```

The resulting plot shows, that the mean arrival delay for the long distance flights for both airlines are neither greater or less then the delay for short distance flights. Thus, longer flights do not have longer delays on average.

### 3.4 Chapter of choice
In the following, we visualize the airport locations a the US map and implement the shinyApp for interactive plots.

#### 3.4.1 Airport location on map

Because we merged the coordinates of the airport data set to our flights data set, we can now visualize the airports in a map. We got the Cartographic Boundary of the USA from the webpage "Cartographic Boundary Files - Shapefile"[^2]

[^2]: https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.2015.html#list-tab-1556094155

Considering that we are working with two different airlines, we pointed the airports where each airline origins from with a different color. JetBlue is marked with a blue dot, and Alaskan with a yellow dot. There of course are also airports from which both airlines fly from, which are marked with a blue and red dot. 
...
#### 3.4.2 Display difference in flight delay by month using shinyApp

Using {shiny} to create interactive plots.
```{r shiny, echo = TRUE, include= FALSE}
...
![Interactive plot to display delay of Alaska and JetBlue airline in a selected month using shinyApp](ShinyAppScreenshot.png)
...
James has concerns about his flight being cancelled for his next important trip. Let us look at the cancellation rate for both airlines in 2015.
...
The result shows that in 2015, neither Alaska nor JetBlue airlines has cancelled any of their flights. This adds trustworthiness to both airlines. James's should not be worried about cancellation of his flight whether he is flying with Alaska or JetBlue, as the probability of a flight being cancelled is basically zero. 
...
In the following, we conduct t-tests for the delay difference in each month of 2015. The p-value of the results for the corresponding month are the following:
...
Except for the month of October, where there is no data available to perform a test, a t-test is applied to investigate the delay difference in each month.

All months, except for January (p-value = 0.1832), show a significant delay difference of the two airlines. When looking at the mean of x and y of the Welch Two Sample t-test result, which correspond to the mean delay of Alaska and JetBlue airlines, these months have a lower x than y value. 
...
In the following, we would like to see if we increase the significance level from 5 to 10%, if the delay would become significant. 
...
```
There is still no statistical difference between the delays of both airlines. The results of both t-tests suggest, that the delay difference of the two airlines in January 2015 is rather due to randomness. Thus, it does not matter, which airline Jim chooses for his travel from SAN to BOS in January. However, we recommend Jim to fly with Alaska, as the analysis shows that Alaska has less delay in all other cases.

## 5. Summary
In this project, we conducted an analysis on flight delays focusing on routes that are relevant to Jim's business travels. Our goal was to determine which airline Jim should choose for specific routes or month of the year in order to reduce delays at arrival airports. We found that Alaska and JetBlue operate flights for these routes, so we investigated the delays of these airlines wrt. the routes. With the help of our analysis, Jim is able to make data-based decisions on the choice of the airline regarding his business travels. 

In order to determine the approximate arrival delay, Jim can use the shinyApp to see, how much delay is expected for a specific route in a specific month. In that way, he can better schedule activities after his arrival, e.g. airport pick-up or meetings. 

**Limitation**

It is to mention, that the results might not apply in a year, that is much later then 2015, since the airlines could have a structural change, thus the delays differ.
Furthermore, the t-test requires normality and homogeneity of variance. If the assumptions are not met, the result can be affected.

All in all, we could see, that Alaska airline is expected to have significant less delays in most months. Therefore, we advise Jim to join the loyalty program of Alaska Airlines, to enjoy benefits such as lounge access and also to expect less delays.

## 6. Generative AI
We initially used Generative AI to brainstorm suitable topics for the assignment. It didn't directly lead us to our final topic, but it gave us some ideas in which direction to go to and pushed us to think about, what we should to include in our analysis.

For the coding part, it was very helpful to use generative AI like ChatGPT. It not only helped us to understand the basics of what we needed to do, it also gave us hints on what parts of the program we should further investigate, when there is an error. This helped us to efficiently work on the project without spending most of the time resolving the errors.
